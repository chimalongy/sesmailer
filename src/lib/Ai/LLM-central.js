...
export async function analyzeDomainForScraping(domainName) {
  ...
  const scrapePrompt = result?.scrapePrompt || `You are an expert lead generation and market research agent. Search Google Maps first and then Google to find local ${niche} and ${service} providing these services in ${location}. Extract their business names, direct website URLs, business owner's name, and business owner's email address. If the business owner's email is not directly available on Google or their website, check the business Facebook page to find their official contact email address. The business website URL is COMPULSORY and must exist for every record; other fields may be left empty if not found.`;
